...
public:
    /**
     * @brief Get the pointer to the thread pool that owns the current thread.
     * If this thread belongs to a `BS::thread_pool` object, a pointer to that
     * object will be returned. Otherwise, for example if this thread is the
     * main thread or an independent `std::thread`, `std::nullopt` will be
     * returned.
     *
...
public:
    ...
    // elements stored in a `multi_future` are futures, which cannot be copied.
     ...
     * `multi_future`, rethrowing any stored exceptions.
     *
     ...
     * waiting after the specified duration has passed. This function first
    ...
    {
        const std::chrono::time_point<std::chrono::steady_clock> kStartTime =
            std::chrono::steady_clock::now();
        for (const std::future<T>& future : *this) {
            future.wait_for(duration -
                            (std::chrono::steady_clock::now() - kStartTime));
            if (duration < std::chrono::steady_clock::now() - kStartTime) {
                return false;
            }
        }
        return true;
...
public:
     ...
     */
    explicit ThreadPool(const concurrency_t kNumThreads)
        : ThreadPool(kNumThreads, [] {})
    {
    }
     ...
     * function.
     *
     * @param init_task An initialization function to run in each thread before
     * it starts to execute any submitted tasks. The function must take no
     * arguments and have no return value. It will only be executed exactly
     * once, when the thread is first constructed.
     */
    explicit ThreadPool(const std::function<void()>& init_task)
        : ThreadPool(0, init_task)
    {
    }
     ...
     * once, when the thread is first constructed.
     */
    ThreadPool(const concurrency_t kNumThreads,
               const std::function<void()>& init_task)
        : thread_count(determine_thread_count(kNumThreads)),
          threads(std::make_unique<std::thread[]>(
              determine_thread_count(kNumThreads)))
    {
     ...
     * task queue, with the specified priority. To push a function with
     ...
     * the block, so that it is only called only once per block, but it is up to
     * the user make sure the block function correctly deals with all the
     * indices in each block. Does not return a `multi_future`, so the user must
    ...
    void detach_blocks(
        const T kFirstIndex, const T kIndexAfterLast, F&& block,
        const size_t kNumBlocks = 0 BS_THREAD_POOL_PRIORITY_INPUT)
    {
        if (kIndexAfterLast > kFirstIndex) {
            const Blocks kBlks(kFirstIndex, kIndexAfterLast,
                               kNumBlocks ? kNumBlocks : thread_count);
            for (size_t blk = 0; blk < kBlks.get_num_blocks(); ++blk) {
                detach_task([block = std::forward<F>(block),
                             start = kBlks.start(blk), end = kBlks.end(blk)] {
                    block(start, end);
     ...
     * it is called many times per block. Does not return a `multi_future`, so
     * the user must use `wait()` or some other method to ensure that the loop
     ...
     */
    template <typename T, typename F>
    void detach_loop(const T kFirstIndex, const T kIndexAfterLast, F&& loop,
                     const size_t kNumBlocks = 0 BS_THREAD_POOL_PRIORITY_INPUT)
    {
        if (kIndexAfterLast > kFirstIndex) {
            const Blocks kBlks(kFirstIndex, kIndexAfterLast,
     ...
     * with the specified priority. Does not return a `multi_future`, so the
     * user must use `wait()` or some other method to ensure that the sequence
     * finishes executing, otherwise bad things will happen.
     *
     * @tparam T The type of the indices. Should be a signed or unsigned
     * integer.
     ...
     */
    template <typename T, typename F>
    void detach_sequence(const T kFirstIndex, const T kIndexAfterLast,
                         F&& sequence BS_THREAD_POOL_PRIORITY_INPUT)
    {
        for (T i = kFirstIndex; i < kIndexAfterLast; ++i) {
     ...
     * @param num_threads The number of threads to use.
     */
    void reset(const concurrency_t kNumThreads)
    {
        reset(kNumThreads, [] {});
    }
    ...
    {
        const std::shared_ptr<std::promise<R>> kTaskPromise =
            std::make_shared<std::promise<R>>();
        detach_task([task = std::forward<F>(task), kTaskPromise] {
#ifndef BS_THREAD_POOL_DISABLE_EXCEPTION_HANDLING
            try {
#endif
                if constexpr (std::is_void_v<R>) {
                    task();
                    kTaskPromise->set_value();
                } else {
                    kTaskPromise->set_value(task());
                }
#ifndef BS_THREAD_POOL_DISABLE_EXCEPTION_HANDLING
            } catch (...) {
                try {
                    kTaskPromise->set_exception(std::current_exception());
                } catch (...) {
                }
            }
#endif
        } BS_THREAD_POOL_PRIORITY_OUTPUT);
        return kTaskPromise->get_future();
    }
     ...
     * the block, so that it is only called only once per block, but it is up to
     * the user make sure the block function correctly deals with all the
     * indices in each block. Returns a `multi_future` that contains the futures
     ...
     * inclusive. In other words, it will be equivalent to `for (T i =
     * first_index; i < index_after_last; ++i)`. Note that if `index_after_last
     * <= first_index`, no blocks will be submitted, and an empty `multi_future`
     * will be returned.
     * @param block A function that will be called once per block. Should take
     * exactly two arguments: the first index in the block and the index after
     * the last index in the block. `block(start, end)` should typically involve
        ...
        const size_t kNumBlocks = 0 BS_THREAD_POOL_PRIORITY_INPUT)
    {
        if (kIndexAfterLast > kFirstIndex) {
            ...
            future.reserve(kBlks.get_num_blocks());
            for (size_t blk = 0; blk < kBlks.get_num_blocks(); ++blk) {
                future.push_back(submit_task([block = std::forward<F>(block),
                                              start = kBlks.start(blk),
                                              end = kBlks.end(blk)] {
     ...
     * a `multi_future` that contains the futures for all of the blocks.
     *
     * @tparam T The type of the indices. Should be a signed or unsigned
     * integer.
     * @tparam F The type of the function to loop through.
     ...
     * inclusive. In other words, it will be equivalent to `for (T i =
     * first_index; i < index_after_last; ++i)`. Note that if `index_after_last
     * <= first_index`, no tasks will be submitted, and an empty `multi_future`
     * will be returned.
     * @param loop The function to loop through. Will be called once per index,
     * many times per block. Should take exactly one argument: the loop index.
     ...
     */
    template <typename T, typename F>
    [[nodiscard]] MultiFuture<void> submit_loop(
        const T kFirstIndex, const T kIndexAfterLast, F&& loop,
        const size_t kNumBlocks = 0 BS_THREAD_POOL_PRIORITY_INPUT)
    {
        if (kIndexAfterLast > kFirstIndex) {
            const Blocks kBlks(kFirstIndex, kIndexAfterLast,
    ...
    /**
     * @brief Submit a sequence of tasks enumerated by indices to the queue,
     * with the specified priority. Returns a `multi_future` that contains the
     * futures for all of the tasks.
     *
     * @tparam T The type of the indices. Should be a signed or unsigned
     * integer.
     * @tparam F The type of the function used to define the sequence.
    ...
    [[nodiscard]] MultiFuture<R> submit_sequence(
        const T kFirstIndex, const T kIndexAfterLast,
        F&& sequence BS_THREAD_POOL_PRIORITY_INPUT)
    {
        if (kIndexAfterLast > kFirstIndex) {
            MultiFuture<R> future;
            future.reserve(static_cast<size_t>(kIndexAfterLast - kFirstIndex));
            for (T i = kFirstIndex; i < kIndexAfterLast; ++i) {
                future.push_back(
...
private:
     ...
     *
     * @param init_task An initialization function to run in each thread before
     * it starts to execute any submitted tasks.
     */
    ...
    [[nodiscard]] static concurrency_t determine_thread_count(
        const concurrency_t kNumThreads)
    {
        if (kNumThreads > 0) {
            return kNumThreads;
        }
     ...
     * @param init_task An initialization function to run in this thread before
     * it starts to execute any submitted tasks.
     */
    void worker(const concurrency_t kIdx,
                const std::function<void()>& init_task)
    {
        this_thread::get_index.index = kIdx;
        ...
        while (true) {
            ...
            if (waiting && (tasks_running == 0) &&
                BS_THREAD_POOL_PAUSED_OR_EMPTY) {
            ...
            }
            {
                ...
                tasks.pop();
#else
                const std::function<void()> kTask = std::move(tasks.front());
                tasks.pop();
#endif
                ++tasks_running;
                tasks_lock.unlock();
    ...
    public:
         ...
         * into.
         */
        Blocks(const T kFirstIndex, const T kIndexAfterLast,
               const size_t kNumBlocks)
            : first_index(kFirstIndex),
              index_after_last(kIndexAfterLast),
              num_blocks(kNumBlocks)
        {
            if (index_after_last > first_index) {
                const auto kTotalSize =
                    static_cast<size_t>(index_after_last - first_index);
                if (num_blocks > kTotalSize) {
                    num_blocks = kTotalSize;
                }
                block_size = kTotalSize / num_blocks;
                remainder = kTotalSize % num_blocks;
                if (block_size == 0) {
                    block_size = 1;
                    num_blocks = (kTotalSize > 1) ? kTotalSize : 1;
                }
            } else {
                num_blocks = 0;
            }
         ...
         */
        [[nodiscard]] T start(const size_t kBlock) const
        {
            return first_index + static_cast<T>(kBlock * block_size) +
                   static_cast<T>(kBlock < remainder ? kBlock : remainder);
        }
         ...
         */
        [[nodiscard]] T end(const size_t kBlock) const
        {
            return (kBlock == num_blocks - 1) ? index_after_last
                                              : start(kBlock + 1);
        }
    ...
    public:
        ...
        /**
         * @brief Construct a new task with an assigned priority by moving the
         * task.
         *
         * @param task_ The task.
         * @param priority_ The desired priority.
    ...
    /**
     * @brief A condition variable to notify `worker()` that a new task has
     * become available.
     */
